...
void	init_data(t_wc_data *data, t_info *info, int x)
{
	...
	data->ray_dir[X] = info->vec_dir[X] + info->vec_plane[X] * data->camera_x;
	data->ray_dir[Y] = info->vec_dir[Y] + info->vec_plane[Y] * data->camera_x;
	data->vec_map[X] = (int)info->vec_pos[X];
	data->vec_map[Y] = (int)info->vec_pos[Y];
